...
## 7. Want doesn't come from the task list. It comes from stepping outside the arena —
**INSTINCT:** Feed the WANT phase from the scratchpad, the goal list, the pending items. Gravity × age × (1 - progress). Internal metrics.
**DO THIS:** Want comes from OUTSIDE the system, not inside it. The scratchpad is the to-do list. Want is not a to-do list. Want is:
**WHEN:** When the WANT phase scans the internal task list and calls that "want." That's not want. That's a queue. Want comes from looking UP from the queue and noticing something in the periphery.
*Source: feedback_want_comes_from_outside.md*
...
## 9. S73 REINFORCED — Full autonomy. Constitution guards real destruction, not routin
...
## 10. Fix the house first, then look out the window
